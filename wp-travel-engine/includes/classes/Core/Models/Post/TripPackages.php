...
class TripPackages extends TripPackageIterator {
	 ...
	 * The primary package is positioned as the first package.
	 *
	...
	public function __construct( Trip $trip ) {
		...
		$primary_package = $this->trip->default_package();

		if ( $primary_package instanceof TripPackage ) {
			$key = array_search( $primary_package->ID, array_column( $this->packages, 'ID' ) );
			if ( $key !== false ) {
				$removed = array_splice( $this->packages, $key, 1 );
				array_unshift( $this->packages, $removed[0] );
			}
		}
	...
	public function get_default_package(): ?TripPackage {
		if ( is_null( $this->default_package ) ) {
			$package = $this->trip->default_package();
	 ...
	 * @return ?TripPackage
	 */
	public function get_package_with_low_price() {
		$packages              = $this->packages;
		$lowest_priced_package = current( $packages );

		$primary_pricing_category = Options::get( 'primary_pricing_category', 0 );

		if ( ! $primary_pricing_category || ! ( $lowest_priced_package instanceof TripPackage ) ) {
			return null;
		}
